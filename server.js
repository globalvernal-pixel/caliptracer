...
// 1.5 Get duplicate student entries (by register number and name)
app.get('/api/students/duplicates', async (req, res) => {
  try {
    ...
    const duplicateRegs = Object.entries(regMap)
      .filter(([_, list]) => list.length > 1)
      .map(([reg, list]) => ({ registerNumber: reg, count: list.length, records: list }));

    const duplicateNames = Object.entries(nameMap)
      .filter(([_, list]) => list.length > 1)
      .map(([_, list]) => ({ name: list[0].name, count: list.length, records: list }));

    res.json({
      totalStudents: students.length,
      duplicateRegsCount: duplicateRegs.length,
    ...
    console.error(err);
    res.status(500).json({ error: 'Failed to fetch duplicate students', details: err.message });
  }
});
